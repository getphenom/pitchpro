...
import { Send, Loader2, UtensilsCrossed, MessageCircle, Plus } from "lucide-react";
      ...
      >
        {!isUser && (
          <div className="flex items-center gap-1.5 mb-1">
            ...
            <span className="text-[10px] font-semibold text-green-400">Nutrition Coach</span>
          </div>
        )}
        {message.content &&
          (isUser ? (
            ...
            <div className="prose prose-sm prose-invert max-w-none text-sm [&_h2]:text-base [&_h2]:font-heading [&_h2]:font-bold [&_h2]:text-green-400 [&_h3]:text-sm [&_h3]:font-semibold [&_ul]:pl-4 [&_li]:text-xs [&_p]:text-xs [&_strong]:text-foreground">
...
export default function NutritionCoachChat({ profile }) {
  ...
  const { data: conversations = [] } = useQuery({
    queryKey: ["nutrition-coach-convs"],
    queryFn: () => base44.agents.listConversations({ agent_name: "nutrition_coach" }),
  });
  ...
  const startNewChat = async () => {
    ...
    const conv = await base44.agents.createConversation({
      ...
      metadata: { name: `Nutrition Check — ${new Date().toLocaleDateString()}`, description: "Dietary recovery and nutrition coaching" },
    });
    ...
    return (
      <div className="space-y-4">
        ...
        <p className="text-xs text-muted-foreground">
          An AI sports nutritionist that reviews your meals and training to give personalized recovery advice.
        </p>
          ...
          <div className="space-y-2">
            ...
            {conversations.map((conv) => (
              <button
                ...
                className="w-full text-left p-3 rounded-xl bg-card border border-border hover:border-green-500/30 transition-all"
              >
                <div className="flex items-center gap-2">
                  <MessageCircle className="w-4 h-4 text-green-400" />
                  <div>
                    <p className="text-sm font-medium">{conv.metadata?.name || "Nutrition Session"}</p>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="font-heading font-bold text-sm tracking-wider uppercase text-muted-foreground">🥗 Nutrition Coach</h3>
      ...
      <div ref={scrollRef} className="h-64 overflow-y-auto space-y-3 pr-1 rounded-xl bg-background/50 border border-border p-3">
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center gap-2">
            <UtensilsCrossed className="w-8 h-8 text-green-400/50" />
            <p className="text-xs text-muted-foreground">Ask me to review today's meals, hydration, or give recovery food ideas!</p>
          </div>
        ) : (
          <AnimatePresence>
            {messages.map((msg, i) => (
          ...
          <div className="flex justify-start">
            <div className="bg-card border border-border rounded-2xl rounded-bl-md px-4 py-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-muted-foreground">Analyzing your nutrition...</span>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="flex gap-2">
        <Input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={handleKeyDown} placeholder="e.g. What should I eat after training today?" className="bg-secondary border-border text-sm" disabled={loading} />
        <Button size="icon" disabled={!input.trim() || loading} onClick={handleSend} className="bg-green-600 hover:bg-green-700 flex-shrink-0">

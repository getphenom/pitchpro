...
import { Loader2, X, Clock, TrendingUp, TrendingDown, Plus, Trash2, CheckCircle2, Circle, Dumbbell } from "lucide-react";
...
export default function TestDetailDialog({ open, onClose, test, results = [], onLog }) {
  ...
  const handleSubmit = async () => {
    // Calculate result from sets: sum of (reps × weight) or total value
    let totalValue = 0;
    let hasData = false;
    sets.forEach((s) => {
  ...
  const allDone = sets.every((s) => s.reps && s.done);
    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="bg-card w-full sm:max-w-md max-h-[85vh] rounded-t-2xl sm:rounded-2xl flex flex-col overflow-hidden"
        >
          {/* Header */}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] text-muted-foreground">
                  Record {test.unit} · {test.benchmark?.[0]}–{test.benchmark?.[3]} {test.unit}
                </p>
              </div>
            </div>
